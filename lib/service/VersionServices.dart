...
import 'package:grip/service/UpdateAvailablePopUp.dart';
...
class VersionService {
  ...
  getVersionData(context,value) async {
    print("CheckVersinCalls---${value.androidVersionCode}");
    ...
    if (Platform.isAndroid) {
      ...
      if (currentBuildNumberAndroid != 0 && currentBuildNumberAndroid < liveBuildNumber) {
        ...
        if (value.androidForceUpdate.toString() == "1") {
          ...
          showDialog(context: context, builder: (context) => UpdateAvailable(force: true,storeUrl:value.playstoreUrl.toString()), barrierDismissible: false);
        } else {
      ...
      if (isVersionGreater(value.iosVersion.toString(), packageInfo.data['version'].toString())) {
        ...
        if (value.iosForceUpdate.toString() == "1") {
          //   update force
          showDialog(context: context, builder: (context) => UpdateAvailable(force: true,storeUrl: value.appstoreUrl.toString()), barrierDismissible: false);
        } else {
          //   optional update suggest only skip-able
          showDialog(
            context: context,
